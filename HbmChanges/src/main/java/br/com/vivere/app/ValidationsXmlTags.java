...
public class ValidationsXmlTags {
	...
	private static List<String> listaEntidadesSemSequence = new ArrayList<String>(
			Arrays.asList(
					"TBG_BANCOS","TBG_LOJA","ADM_MENU","SIS_COLUNAS","PRT_EXCEC_PROCESSA"
			)
	);
	...
	private static void inserirTagsChaveComposta(Document doc,Node classElement, File arquivoHbm) {
		...
		if(compositeIdList != null && compositeIdList.getLength() > 0) {
			...
			String nomeFuncional = attrClassComp.getTextContent().replace(nomeTabelaHbmId, "domain."+nomesFuncionaisMap.get(nomeTabelaHbm).concat("Id"));
